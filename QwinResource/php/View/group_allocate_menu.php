...
<form action="<?php url(array('admin', 'group', 'allocate_menu'))?>" method="post">
...
?>
	<tr class="namespace field_value">
		<td><input type="checkbox" name="menu[<?php echo $val['id']?>]" value="1"<?php echo $check?> />
			<?php echo $val['title']?></td>
	</tr>
	<tr class="field_value">
		<td><div class="controller"></div>
<?php
	foreach($menu_list as &$val_2)
	{
		if($val_2['category_id'] == $val['id'])
		{
			if($menu_check[$val_2['id']] == 1)
			{
				$check = ' checked="checked"';
			} else {
				$check = '';
...
?>
			<input type="checkbox" name="menu[<?php echo $val_2['id']?>]" value="1"<?php echo $check?> />
			<a href="<?php echo $val_2['url']?>"><?php echo $val_2['title']?></a>
<?php
			unset($val_2);
		}
	}
