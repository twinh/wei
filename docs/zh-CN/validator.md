验证器
=====
为了保证数据的安全合法,Widget提供了一系列简洁高效的验证器,用于检查数据的正确性.目前包括以下这些验证器:
...
* [Required](#required) - 检查数据是否为空(用于组合验证,如果允许为空且数据为空,则不对数据进行剩余规则的校验)
...
* [File](#file) - 检查数据是否为合法的文件
* [Image](#image) - 检查数据是否为合法的图片
...
如果$type的值不在下表中,将检查数据是否为$type的实例化对象($input instanceof $type)
