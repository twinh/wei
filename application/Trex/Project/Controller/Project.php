...
class Trex_Project_Controller_Project extends Trex_ActionController
{
    ...
    public function convertListOperation($value, $name, $data, $copyData)
    {
        ...
        return Qwin_Helper_Html::jQueryButton($this->url->createUrl($this->_set, array('controller' => 'Ticket', 'action' => 'Add', '_data[project_id]' => $copyData[$primaryKey])), $this->_lang->t('LBL_ACTION_ADD_TICKET'), 'ui-icon-plusthick')
                . Qwin_Helper_Html::jQueryButton($this->url->createUrl($this->_set, array('controller' => 'Status', 'action' => 'Index', 'searchField' => 'project_id', 'searchValue' => $copyData['id'])), $this->_lang->t('LBL_ACTION_VIEW_STATUS'), 'ui-icon-lightbulb')
